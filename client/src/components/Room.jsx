...
// --- CONFIGURATION ---
const DEVELOPER_NAME = "Voly & Farnex"; 
...
function Room() {
  ...
  const addAudioStream = (audio, stream) => { audio.srcObject = stream; audio.addEventListener('loadedmetadata', () => { audio.play(); if (selectedOutputId && audio.setSinkId) audio.setSinkId(selectedOutputId); }); document.body.append(audio); remoteAudioElements.current.push(audio); };
  const toggleMute = () => { if (myStream) { const audioTrack = myStream.getAudioTracks()[0]; audioTrack.enabled = !audioTrack.enabled; setIsMuted(!audioTrack.enabled); } };
  ...
  if (!isPseudoValid) {
    return (
        <div className="flex h-screen bg-[#0a0a0a] items-center justify-center font-sans text-white relative">
            <div className="bg-[#141414] p-8 rounded-2xl shadow-2xl border border-gray-800 w-full max-w-md text-center">
                <div className="flex justify-center mb-6"><div className="bg-red-600/20 p-4 rounded-full"><User size={48} className="text-red-600" /></div></div>
                <h2 className="text-2xl font-bold mb-2">WatchParty</h2>
                <p className="text-gray-400 mb-6 text-sm">Choisis un pseudo pour rejoindre tes amis.</p>
                <input type="text" value={pseudo} onChange={(e) => setPseudo(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleJoin()} placeholder="Ton pseudo..." className="w-full bg-[#0f0f0f] text-white rounded-lg px-4 py-3 border border-gray-700 focus:border-red-600 focus:outline-none mb-4 transition-colors text-center font-bold"/>
                <button onClick={handleJoin} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg transition-colors cursor-pointer">Rejoindre la salle</button>
            </div>
        </div>
    );
  }
    ...
    <div className="flex h-screen flex-col md:flex-row bg-[#0a0a0a] overflow-hidden font-sans text-white">
      <div className="w-full h-[35vh] md:h-full md:flex-1 flex flex-col relative bg-black group z-10">
        <div className="absolute top-0 left-0 w-full p-2 md:p-4 z-30 flex justify-between items-center bg-gradient-to-b from-black/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="flex items-center gap-4">
                ...
                <button onClick={() => setShowVideoInput(!showVideoInput)} className="bg-white/10 hover:bg-white/20 text-white px-3 py-1 rounded-full text-xs md:text-sm flex items-center gap-2 backdrop-blur-md transition border border-white/5 cursor-pointer"><PlusCircle size={14}/> <span className="hidden md:inline">Changer Vidéo</span></button>
                 <button onClick={() => setShowHistory(!showHistory)} className="bg-white/10 hover:bg-white/20 text-white px-3 py-1 rounded-full text-xs md:text-sm flex items-center gap-2 backdrop-blur-md transition border border-white/5 cursor-pointer relative"><History size={14}/> <span className="hidden md:inline">Historique</span><span className="bg-red-600 text-[9px] rounded-full w-4 h-4 flex items-center justify-center absolute -top-1 -right-1">{history.length}</span></button>
            </div>
        ...
        {showVideoInput && (
            <div className="absolute top-16 left-1/2 -translate-x-1/2 z-40 bg-[#1a1a1a] p-3 rounded-xl border border-gray-700 shadow-2xl flex gap-2 w-[90%] md:w-[500px] animate-fade-in">
                ...
                <button onClick={() => changeVideo(newVideoInput)} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-bold">Lancer</button>
            </div>
        )}
        ...
        {isYouTube ? ( <YouTube key={videoUrl} videoId={getYouTubeID(videoUrl)} opts={{ height: '100%', width: '100%', playerVars: { autoplay: 1, controls: 1, playsinline: 1, origin: window.location.origin }}} onReady={onReady} onStateChange={onStateChange} className="w-full h-full absolute top-0 left-0"/> ) : ( <video key={videoUrl} id="html5-player" src={videoUrl} controls playsInline className="w-full h-full max-h-screen object-contain" onPlay={() => !isRemoteUpdate.current && socket.emit("video_action", { roomId, type: 'play' })} onPause={() => !isRemoteUpdate.current && socket.emit("video_action", { roomId, type: 'pause' })}/> )}
        </div>
      </div>
      ...
      <div className="flex-1 md:flex-none md:w-80 bg-[#141414] border-t md:border-t-0 md:border-l border-gray-800 flex flex-col z-20 shadow-2xl relative h-full">
        <div className="p-3 md:p-4 border-b border-gray-800 bg-[#1a1a1a] flex justify-between shadow-sm items-center">
            ...
            <div className="flex gap-2"><button onClick={() => setShowSettings(!showSettings)} className={`p-2 rounded-full transition-all ${showSettings ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}><Settings size={18}/></button><button onClick={toggleMute} className={`p-2 rounded-full transition-all duration-200 shadow-md ${isMuted ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-500'}`}>{isMuted ? <MicOff size={18}/> : <Mic size={18}/>}</button></div>
        </div>

        {showSettings && (
            <div className="bg-gray-800 p-4 border-b border-gray-700 animate-fade-in absolute top-14 left-0 w-full z-50 shadow-xl">
                <div><label className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-2 flex items-center gap-2"><Mic size={12}/> Microphone</label><select value={selectedInputId} onChange={(e) => changeMicrophone(e.target.value)} className="w-full bg-[#0f0f0f] text-white text-xs rounded border border-gray-600 p-2 focus:border-red-600 focus:outline-none">{audioInputs.map(device => (<option key={device.deviceId} value={device.deviceId}>{device.label || `Microphone...`}</option>))}</select></div>
                {audioOutputs.length > 0 && (<div className="mt-2"><label className="text-xs text-gray-400 font-bold uppercase tracking-wider mb-2 flex items-center gap-2"><Volume2 size={12}/> Sortie</label><select value={selectedOutputId} onChange={(e) => changeSpeaker(e.target.value)} className="w-full bg-[#0f0f0f] text-white text-xs rounded border border-gray-600 p-2 focus:border-red-600 focus:outline-none">{audioOutputs.map(device => (<option key={device.deviceId} value={device.deviceId}>{device.label || `Speaker...`}</option>))}</select></div>)}
                <div className="pt-2 border-t border-gray-700 mt-2"><label className="flex items-center gap-2 cursor-pointer hover:text-white text-gray-400 transition"><input type="checkbox" className="rounded bg-gray-700 border-gray-600 text-red-600" onChange={(e) => {if (e.target.checked && myStream) {const audio = document.createElement('audio'); audio.id = "mic-test-feedback"; audio.srcObject = myStream; if (selectedOutputId && audio.setSinkId) audio.setSinkId(selectedOutputId); audio.play();} else { const audio = document.getElementById("mic-test-feedback"); if (audio) { audio.pause(); audio.remove(); }}}}/> <span className="text-xs font-bold uppercase tracking-wider">Entendre ma voix (Test)</span></label></div>
            </div>
        )}

        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#141414]">
          {messages.map((msg, i) => {
            const isMe = msg.author === pseudo; const isSystem = msg.author === "Système";
            if (isSystem) return <div key={i} className="text-center text-[10px] text-gray-500 italic my-2">-- {msg.message} --</div>;
            return ( <div key={i} className={`flex flex-col ${isMe ? "items-end" : "items-start"} animate-fade-in`}> {!isMe && <span className="text-[10px] text-gray-400 ml-2 mb-1">{msg.author}</span>} <div className={`px-3 py-2 rounded-2xl text-sm max-w-[85%] break-words shadow-sm ${isMe ? "bg-red-600 text-white rounded-tr-none" : "bg-gray-800 text-gray-200 rounded-tl-none"}`}> {msg.message} </div> <span className="text-[9px] text-gray-600 mt-1 px-1 opacity-60">{msg.time}</span> </div> );
          })}
        </div>
